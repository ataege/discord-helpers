...
import { ClientData, ICommand, EventHandler, IService } from '..';
...
export class Client extends DjsClient {
  ...
  constructor(options: ClientOptions, data: ClientData) {
    super(options);
    this.prefix = data.prefix;
    this.commandsPath = data.commandsPath;
    ...
    glob(this.commandsPath, (err, matches) => {
      for (const match of matches) {
        ...
        try {
          ...
          this.logger.info(`${command.name} initialized ${!command.aliases ? '' : `with aliases: ${command.aliases}`}`);
        } catch {}
      }
    });
  }
